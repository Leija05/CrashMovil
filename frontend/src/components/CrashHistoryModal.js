...
export default function CrashHistoryModal({ open, onClose }) {
  ...
  const resetFilters = () => {
    ...
    const t = new Date();
    setDateFrom(
      new Date(t.getTime() - 30 * 86400 * 1000).toISOString().slice(0, 10),
    );
  ...
  return (
    <div
      className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-2 sm:p-3 lg:p-6"
          ...
          <div className="lg:col-span-3 border-b lg:border-b-0 lg:border-r border-white/10 relative h-[45vh] min-h-[280px] lg:h-auto lg:min-h-0">
            {withGps.length > 0 ? (
              ...
              <div className="h-full flex items-center justify-center text-neutral-500 text-xs uppercase tracking-[0.3em]">
                {loading
                  ? "Cargando..."
                  : "Sin choques con GPS para los filtros actuales"}
              </div>
            )}
            ...
            >
              {loading ? (
                ...
                <div className="text-xs text-neutral-500 px-2">
                  Sin choques para los filtros aplicados.
                </div>
              ) : (
                impacts.map((i) => {
                  const isSel = i.id === selectedId;
                  ...
                  return (
                    <div
                      ...
                      ref={(el) => {
                        if (el) cardRefs.current[i.id] = el;
                      }}
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            />
                            <span
                              className={`text-[9px] font-semibold uppercase tracking-[0.2em] px-1.5 py-0.5 rounded border ${STATUS_BADGE[i.status]}`}
                            >
                              {STATUS_LABEL[i.status]}
                            </span>
